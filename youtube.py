...
def _parse_video_id(url: str) -> Optional[str]:
    ...
    if parsed_url.netloc == "youtu.be":
        video_id = parsed_url.path.lstrip("/")  # 直接从路径部分提取
    else:
        # 处理 www.youtube.com/watch?v=VIDEO_ID 格式
        query = parse_qs(parsed_url.query)
        if "v" in query:
            video_id = query["v"][0]  # 提取视频 ID
        else:
            return None
...
class YoutubeLoader(BaseLoader):
    ...
    @classmethod
    def from_youtube_url(cls, youtube_url: str, api_key: str, **kwargs: Any) -> YoutubeLoader:
        """Given a YouTube URL, construct a loader.
        See `YoutubeLoader()` constructor for a list of keyword arguments.
        """
        video_id = cls.extract_video_id(youtube_url)

        return cls(video_id, api_key, **kwargs)
    ...
    def load(self) -> List[Document]:
        ...
        if self.add_video_info:
            ...
            video_info = self._get_video_infos()
    ...
    def _get_video_infos(self) -> Dict:
        ...
        video_info = {
            ...
            "description": snippet.get("description", "Unknown"),
            "view_count": statistics.get("viewCount", 0),
            "thumbnail_url": snippet["thumbnails"]["default"]["url"],
            "publish_date": snippet.get("publishedAt", "Unknown"),
            "author": snippet.get("channelTitle", "Unknown"),
            "length": content_details.get("duration", "Unknown")  # Duration in ISO 8601 format
        }
